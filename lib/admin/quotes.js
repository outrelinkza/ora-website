    ...
    if (supabase) {
      try {
        ...
        if (!quotesError && quotesData) {
          ...
          // Get recent quotes (last 50 for better overview)
          quotes = quotesData.slice(0, 50).map(quote => ({
            id: quote.id,
            quoteName: quote.quote_name || 'Untitled Quote',
            platform: quote.platform || 'N/A',
            clientName: quote.client_name || 'Unknown Client',
            status: quote.status || 'draft',
            calculatedRate: quote.calculated_rate || 0,
            followerCount: quote.follower_count || 0,
